...
def create_api_blueprint(app):
    """Create ItcFile blueprint."""
    blueprint = app.extensions["itc"].resource_files.as_blueprint()
    ...
    funcs = [
        ...
        if k.startswith("init_addons_itc_file") and callable(v)
    ]
    for func in funcs:
...
def init_addons_itc_file_published_service(state):
    """Init app."""
    app = state.app
    ext = app.extensions["itc"]
