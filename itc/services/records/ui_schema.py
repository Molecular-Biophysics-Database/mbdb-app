...
class ItcUISchema(UIRequestsSerializationMixin, InvenioRDMUISchema):
    class Meta:
        unknown = ma.RAISE
...
class MethodSpecificParametersUISchema(DictOnlySchema):
    ...
    schema_version = ma_fields.String(required=True, validate=[OneOf(["0.1.1"])])
...
class MeasurementsItemUISchema(DictOnlySchema):
    ...
    sample_in_cell = ma_fields.Nested(lambda: SampleInCellUISchema(), required=True)

    sample_in_syringe = ma_fields.Nested(lambda: SampleInCellUISchema(), required=True)
...
class SampleInCellUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class StirringSpeedUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
