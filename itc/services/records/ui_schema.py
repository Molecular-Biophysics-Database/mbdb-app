...
class ItcUISchema(UIRequestsSerializationMixin, InvenioUISchema):
    class Meta:
        unknown = ma.RAISE
...
class GeneralParametersUISchema(DictOnlySchema):
    ...
    associated_publication = ma_fields.Nested(
        lambda: AssociatedPublicationUISchema(), required=True
    )
...
class ConstituentsItemUISchema(DictOnlySchema):
    ...
    chemical = ma_fields.Nested(lambda: ChemicalChemicalUISchema())
...
class MethodSpecificParametersUISchema(DictOnlySchema):
    ...
    cell_temperature = ma_fields.Nested(lambda: TemperatureUISchema(), required=True)

    cell_volume = ma_fields.Nested(lambda: CellVolumeUISchema(), required=True)

    data_analysis = ma_fields.List(ma_fields.Nested(lambda: DataAnalysisItemUISchema()))

    experiment_type = ma_fields.Nested(lambda: ExperimentTypeUISchema(), required=True)
...
class ExperimentTypeUISchema(DictOnlySchema):
    ...
    injection_parameters = ma_fields.List(
        ma_fields.Nested(lambda: InjectionParametersItemUISchema())
    )
...
class SampleUISchema(DictOnlySchema):
    ...
    chemical_environment = ma_fields.List(
        ma_fields.Nested(lambda: EntityUISchema()), required=True
    )
    ...
    target_in_cell = ma_fields.List(
        ma_fields.Nested(lambda: TargetInCellItemUISchema()), required=True
    )
...
class TitrationUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class DataFittingUISchema(DictOnlySchema):
    ...
    data_fitting_model = ma_fields.Nested(lambda: DataFittingModelUISchema())
...
class InjectionParametersItemUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class Single_injectionUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class TargetInCellItemUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class BySequencingUISchema(DictOnlySchema):
    ...
    coverage = ma_fields.Integer(required=True)
...
class CellVolumeUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE

    unit = ma_fields.String(required=True, validate=[OneOf(["µl", "ml"])])

    value = ma_fields.Float(required=True)


class ChemicalChemicalUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.INCLUDE
    ...
    additional_identifiers = ma_fields.String()

    inchikey = ma_fields.String()

    molecular_formula = ma_fields.String()

    molecular_weight = ma_fields.String()
...
class DataFittingModelUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class ReferencePowerUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
