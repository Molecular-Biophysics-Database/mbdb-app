...
class ItcSchema(RequestsSchemaMixin, BaseRecordSchema):
    class Meta:
        unknown = ma.RAISE
...
class MethodSpecificParametersSchema(DictOnlySchema):
    ...
    experiment_type = ma_fields.Nested(lambda: ExperimentTypeSchema(), required=True)
...
class ExperimentTypeSchema(PolymorphicSchema):
    class Meta:
        unknown = ma.RAISE
...
class ChemicalSchema(DictOnlySchema):
    ...
    chemical = ma_fields.Nested(lambda: ChemicalChemicalSchema())
...
class BySequencingSchema(DictOnlySchema):
    ...
    coverage = ma_fields.Integer(
        required=True, validate=[ma.validate.Range(min=0, max=100)]
    )
...
class ChemicalChemicalSchema(DictOnlySchema):
    class Meta:
        unknown = ma.INCLUDE
