...
def create_api_blueprint(app):
    """Create SprFile blueprint."""
    blueprint = app.extensions["spr"].resource_files.as_blueprint()
    ...
    funcs = [
        ...
        if k.startswith("init_addons_spr_file") and callable(v)
    ]
    for func in funcs:
...
def init_addons_spr_file_requests(state):
    app = state.app
...
def init_addons_spr_file_published_service(state):
    """Init app."""
    app = state.app
    ext = app.extensions["spr"]
