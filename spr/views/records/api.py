...
def create_api_blueprint(app):
    """Create SprRecord blueprint."""
    with app.app_context():
        blueprint = app.extensions["spr"].resource_records.as_blueprint()
        ...
        funcs = [
            ...
            if k.startswith("init_addons_spr") and callable(v)
        ]
        for func in funcs:
...
def init_addons_spr_requests(state):
    ...
    for er in getattr(config, "SPR_ENTITY_RESOLVERS", []):
