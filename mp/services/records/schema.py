...
class MpSchema(RDMBaseRecordSchema):
    class Meta:
        unknown = ma.RAISE

    access = NestedAttribute(lambda: AccessSchema())

    metadata = ma_fields.Nested(lambda: MpMetadataSchema())
...
class MethodSpecificParametersSchema(VersionUpdateSchema):
    ...
    calibrants = ma_fields.List(
        ma_fields.Nested(lambda: CalibrantsItemSchema()),
        required=True,
        validate=[ma.validate.Length(min=1)],
    )
...
class MeasurementsItemSchema(DictOnlySchema):
    ...
    duration = ma_fields.Nested(lambda: DurationSchema(), required=True)
...
class ModeSchema(PolymorphicSchema):
    class Meta:
        unknown = ma.RAISE

    Flow = ma_fields.Nested(lambda: FlowSchema(), required=True)

    Static = ma_fields.Nested(lambda: StaticSchema(), required=True)
...
class CalibrantsItemSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class StaticSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class SampleCarrierSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE

    cleaning_protocol = ma_fields.List(
        ma_fields.Nested(lambda: ProtocolItemSchema()),
        validate=[ma.validate.Length(min=1)],
    )
...
class BufferFlowrateSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class PrimaryLaserWavelengthSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
