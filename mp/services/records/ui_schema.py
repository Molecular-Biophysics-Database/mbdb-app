...
class MpUISchema(UIRequestsSerializationMixin, InvenioRDMUISchema):
    class Meta:
        unknown = ma.RAISE
    ...
    metadata = ma_fields.Nested(lambda: MpMetadataUISchema())
...
class MpMetadataUISchema(Schema):
    class Meta:
        unknown = ma.RAISE
...
class MethodSpecificParametersUISchema(DictOnlySchema):
    ...
    calibrants = ma_fields.List(
        ma_fields.Nested(lambda: CalibrantsItemUISchema()), required=True
    )
...
class MeasurementsItemUISchema(DictOnlySchema):
    ...
    duration = ma_fields.Nested(lambda: DurationUISchema(), required=True)

    name = ma_fields.String(required=True)

    sample = ma_fields.Nested(lambda: SampleUISchema(), required=True)

    sample_dilution = ma_fields.Nested(lambda: SampleDilutionUISchema())
...
class CalibrantsItemUISchema(DictOnlySchema):
    ...
    molecular_weight = ma_fields.Nested(lambda: MolecularWeightUISchema())
...
class Hill_coefficientUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class ModeUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class SampleDilutionUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE

    dilution_buffer = ma_fields.Nested(lambda: DilutionBufferUISchema())

    dilution_factor = ma_fields.Float(required=True)
...
class SampleUISchema(DictOnlySchema):
    ...
    targets = ma_fields.List(
        ma_fields.Nested(lambda: TargetsItemUISchema()), required=True
    )


class Size_of_spherically_represented_entityUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class StaticUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class DilutionBufferUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class FlowUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class SampleCarrierUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE

    cleaning_protocol = ma_fields.List(ma_fields.Nested(lambda: ProtocolItemUISchema()))

    material = ma_fields.String()

    supplier = ma_fields.Nested(lambda: SupplierUISchema())

    surface_modification = ma_fields.String(required=True)
...
class TargetsItemUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class BufferFlowrateUISchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
