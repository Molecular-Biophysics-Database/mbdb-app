...
def create_api_blueprint(app):
    """Create MpRecord blueprint."""
    blueprint = app.extensions["mp"].resource_records.as_blueprint()
    ...
    funcs = globals()
    funcs = [
        v for k, v in funcs.items() if k.startswith("init_addons_mp") and callable(v)
    ]
    for func in funcs:
...
def init_addons_mp_requests(state):
    app = state.app
    requests = app.extensions["invenio-requests"]

    from mp import config

    for er in getattr(config, "MP_ENTITY_RESOLVERS", []):
