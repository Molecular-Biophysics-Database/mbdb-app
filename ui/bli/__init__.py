...
class BliUIResourceConfig(RecordsUIResourceConfig):
    template_folder = "templates"
    url_prefix = "/bli"
    blueprint_name = "bli_ui"
    model_name = "bli"
    search_component = UIComponent(
        "BliResultsListItem",
        "@js/bli/search/ResultsListItem",
...
class BliUIResource(RecordsUIResource):
    pass

def ui_overrides(app):
    """Register UI overrides."""
    ui_resource_config = BliUIResourceConfig()
...
def init_menu(app):
    ...
    with app.app_context():
        current_menu.submenu("plus.create_bli").register(
            f"{ui_resource_config.blueprint_name}.deposit_create",
            _("New Bio-layer interferometry (BLI)"),
            order=1,
...
def create_blueprint(app):
    ...
    blueprint = BliUIResource(BliUIResourceConfig()).as_blueprint()
    return blueprint
