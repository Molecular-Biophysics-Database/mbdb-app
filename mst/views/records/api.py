...
def create_api_blueprint(app):
    """Create MstRecord blueprint."""
    blueprint = app.extensions["mst"].resource_records.as_blueprint()
    ...
    funcs = globals()
    funcs = [
        v for k, v in funcs.items() if k.startswith("init_addons_mst") and callable(v)
    ]
    for func in funcs:
...
def init_addons_mst_requests(state):
    app = state.app
