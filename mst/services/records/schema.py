...
class MstSchema(RequestsSchemaMixin, BaseRecordSchema):
    class Meta:
        unknown = ma.RAISE
...
class MstMetadataSchema(Schema):
    class Meta:
        unknown = ma.RAISE
...
class SampleSchema(DictOnlySchema):
    ...
    ligands = ma_fields.List(
        ma_fields.Nested(lambda: LigandsItemSchema()),
        required=True,
        validate=[ma.validate.Length(min=1)],
    )
...
class LigandsItemSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
...
class MeasuredDataSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE

    x_data = ma_fields.Nested(lambda: XDataSchema(), required=True)

    y_data = ma_fields.Nested(lambda: XDataSchema(), required=True)
...
class FColdAndHotSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
    ...
    f_hot_end = ma_fields.Float(required=True, validate=[ma.validate.Range(min=0.0)])

    f_hot_start = ma_fields.Float(required=True, validate=[ma.validate.Range(min=0.0)])
...
class XDataSchema(DictOnlySchema):
    class Meta:
        unknown = ma.RAISE
