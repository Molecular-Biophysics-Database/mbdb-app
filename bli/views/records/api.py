...
def create_api_blueprint(app):
    """Create BliRecord blueprint."""
    blueprint = app.extensions["bli"].resource_records.as_blueprint()
    ...
    funcs = globals()
    funcs = [
        v for k, v in funcs.items() if k.startswith("init_addons_bli") and callable(v)
    ]
    for func in funcs:
...
def init_addons_bli_requests(state):
    app = state.app
...
def init_addons_bli_published_service(state):
    """Init app."""
    app = state.app
    ext = app.extensions["bli"]
