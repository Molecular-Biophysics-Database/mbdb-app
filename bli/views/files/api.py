...
def create_api_blueprint(app):
    """Create BliFile blueprint."""
    blueprint = app.extensions["bli"].resource_files.as_blueprint()
    ...
    funcs = [
        ...
        if k.startswith("init_addons_bli_file") and callable(v)
    ]
    for func in funcs:
...
def init_addons_bli_file_requests(state):
    app = state.app
...
def init_addons_bli_file_published_service(state):
    """Init app."""
    app = state.app
    ext = app.extensions["bli"]
