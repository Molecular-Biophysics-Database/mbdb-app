...
class BliExt:
    ...
    def service_records(self):
        service_config = build_config(config.BLI_RECORD_SERVICE_CONFIG, self.app)
    ...
    def resource_records(self):
        return config.BLI_RECORD_RESOURCE_CLASS(
            ...
            config=build_config(config.BLI_RECORD_RESOURCE_CONFIG, self.app),
        )
    ...
    def service_files(self):
        service_config = build_config(config.BLI_FILES_SERVICE_CONFIG, self.app)
    ...
    def resource_files(self):
        return config.BLI_FILES_RESOURCE_CLASS(
            ...
            config=build_config(config.BLI_FILES_RESOURCE_CONFIG, self.app),
        )

    @cached_property
    def service_draft_files(self):
        service_config = build_config(config.BLI_DRAFT_FILES_SERVICE_CONFIG, self.app)
    ...
    def resource_draft_files(self):
        return config.BLI_DRAFT_FILES_RESOURCE_CLASS(
            ...
            config=build_config(config.BLI_DRAFT_FILES_RESOURCE_CONFIG, self.app),
        )
...
RDM_MODEL_CONFIG = {  # allows merging stuff from other builders
    ...
    "api_service": "bli.services.records.service.BliService",
    "api_service_config": "bli.services.records.config.BliServiceConfig",
    "api_resource": "bli.resources.records.resource.BliResource",
    "api_resource_config": "bli.resources.records.config.BliResourceConfig",
    "ui_resource_config": "ui.bli.BliUIResourceConfig",
    "record_cls": "bli.records.api.BliRecord",
    "pid_type": "bli",
    "draft_cls": "bli.records.api.BliDraft",
}
