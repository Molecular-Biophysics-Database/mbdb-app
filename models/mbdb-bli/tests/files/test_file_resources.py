...
def test_files_api_flow(client_with_credentials, search_clear, headers, input_data):
    ...
    res = client_with_credentials.get(
        f"/mbdb-bli/{id_}/files/test.pdf", headers=headers
    )
    ...
    res = client_with_credentials.put(
        f"/mbdb-bli/{id_}/files/test.pdf/content",
        headers={
    ...
    res = client_with_credentials.post(
        f"/mbdb-bli/{id_}/files/test.pdf/commit", headers=headers
    )
    assert res.status_code == 200
    ...
    res = client_with_credentials.get(
        f"/mbdb-bli/{id_}/files/test.pdf/content", headers=headers
    )
    assert res.status_code == 200
    ...
    res = client_with_credentials.put(
        f"/mbdb-bli/{id_}/files/test.pdf", headers=headers, json={"title": "New title"}
    )
    assert res.status_code == 200
    assert res.json["key"] == "test.pdf"
    assert res.json["status"] == "completed"
    ...
    # Get all files
    res = client_with_credentials.get(f"/mbdb-bli/{id_}/files", headers=headers)
    assert res.status_code == 200
...
def test_default_preview_file(
    ...
    for f in file_entries:
        res = client_with_credentials.put(
            f"/mbdb-bli/{id_}/files/{f['key']}/content",
            headers={
                "content-type": "application/octet-stream",
                "accept": "application/json",
            },
        ...
        res = client_with_credentials.post(
            f"/mbdb-bli/{id_}/files/{f['key']}/commit", headers=headers
        )
        assert res.status_code == 200
        assert res.json["status"] == "completed"
    ...
    res = client_with_credentials.put(
        f"/mbdb-bli/{id_}", headers=headers, json=input_data
    )
    assert res.status_code == 200
    ...
    res = client_with_credentials.delete(
        f"/mbdb-bli/{id_}/files/f3.pdf", headers=headers
    )
    assert res.status_code == 204
...
def test_file_api_errors(
    ...
    res = client_with_credentials.post(
        f"/mbdb-bli/{id_}/files", headers=headers, json={"key": "test.pdf"}
    )
...
def test_disable_files_when_files_already_present_should_error(
    ...
    client_with_credentials.post(
        f"/mbdb-bli/{id_}/files", headers=headers, json=[{"key": file_id}]
    )
    client_with_credentials.put(
        f"/mbdb-bli/{id_}/files/{file_id}/content",
        headers={
            "content-type": "application/octet-stream",
            "accept": "application/json",
        },
    ...
    client_with_credentials.post(
        f"/mbdb-bli/{id_}/files/{file_id}/commit", headers=headers
    )
    # Disable files
