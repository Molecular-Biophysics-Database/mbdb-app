...
def test_disable_files_when_files_already_present_should_error(
    ...
    response = client_with_credentials.post(
        "/mbdb-bli/", headers=headers, json=input_data
    )
    id_ = response.json["id"]
    # Add file
    file_id = "f1.pdf"
    client_with_credentials.post(
        f"/mbdb-bli/{id_}/draft/files", headers=headers, json=[{"key": file_id}]
    )
    client_with_credentials.put(
        f"/mbdb-bli/{id_}/draft/files/{file_id}/content",
        headers={
            "content-type": "application/octet-stream",
            "accept": "application/json",
        },
    ...
    client_with_credentials.post(
        f"/mbdb-bli/{id_}/draft/files/{file_id}/commit", headers=headers
    )
    # Disable files
