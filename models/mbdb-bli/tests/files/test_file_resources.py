...
def test_files_api_flow(client_with_credentials, search_clear, headers, input_data):
    ...
    # Initialize a draft
    res = client_with_credentials.post("/mbdb-bli/", headers=headers, json=input_data)
    ...
    res = client_with_credentials.get(
        f"/mbdb-bli/{id_}/draft/files/test.pdf", headers=headers
    )
    ...
    res = client_with_credentials.put(
        f"/mbdb-bli/{id_}/draft/files/test.pdf/content",
        headers={
    ...
    res = client_with_credentials.post(
        f"/mbdb-bli/{id_}/draft/files/test.pdf/commit", headers=headers
    )
    assert res.status_code == 200
    ...
    res = client_with_credentials.get(
        f"/mbdb-bli/{id_}/draft/files/test.pdf/content", headers=headers
    )
    assert res.status_code == 200
    ...
    res = client_with_credentials.put(
        f"/mbdb-bli/{id_}/draft/files/test.pdf",
        headers=headers,
    ...
    # Get all files
    res = client_with_credentials.get(f"/mbdb-bli/{id_}/draft/files", headers=headers)
    assert res.status_code == 200
...
def test_default_preview_file(
    ...
    for f in file_entries:
        res = client_with_credentials.put(
            f"/mbdb-bli/{id_}/draft/files/{f['key']}/content",
            headers={
                "content-type": "application/octet-stream",
                "accept": "application/json",
            },
        ...
        res = client_with_credentials.post(
            f"/mbdb-bli/{id_}/draft/files/{f['key']}/commit", headers=headers
        )
        assert res.status_code == 200
        assert res.json["status"] == "completed"
    ...
    res = client_with_credentials.put(
        f"/mbdb-bli/{id_}/draft", headers=headers, json=input_data
    )
    assert res.status_code == 200
    ...
    res = client_with_credentials.delete(
        f"/mbdb-bli/{id_}/draft/files/f3.pdf", headers=headers
    )
    assert res.status_code == 204
...
def test_file_api_errors(
    ...
    res = client_with_credentials.post(
        f"/mbdb-bli/{id_}/draft/files", headers=headers, json={"key": "test.pdf"}
    )
...
def published_id(client, location, headers):
    ...
    # Initialize files upload
    res = client.post(
        f"/mbdb-bli/{id_}/draft/files", headers=h, json=[{"key": "test.pdf"}]
    )
    assert res.status_code == 201
    assert res.json["entries"][0]["key"] == "test.pdf"
    ...
    # Commit the file
    res = client.post(f"/mbdb-bli/{id_}/draft/files/test.pdf/commit", headers=h)
    assert res.status_code == 200
    assert res.json["key"] == "test.pdf"
    assert res.json["status"] == "completed"

    # Publish the record
    res = client.post(f"/mbdb-bli/{id_}/draft/actions/publish", headers=h)
...
def test_files_publish_flow(client, search_clear, location, headers):
    ...
    h = headers
    # Create a draft
    res = client.post("/mbdb-bli/", headers=h, json={"metadata": {"title": "Test"}})
    assert res.status_code == 201
    id_ = res.json["id"]
    ...
    res = client.get(f"/mbdb-bli/{id_}/files", headers=h)
    assert res.status_code == 200
    assert res.json["entries"][0]["key"] == "test.pdf"
    assert res.json["entries"][0]["status"] == "completed"

    # Edit the record
    res = client.post(f"/mbdb-bli/{id_}/draft", headers=h)
    assert res.status_code == 201
...
def test_import_files(client, search_clear, location, headers, published_id):
    ...
    # New version
    res = client.post(f"/mbdb-bli/{id_}/versions", headers=h)
    assert res.status_code == 201
    new_id = res.json["id"]

    # Check new version files
    res = client.get(f"/mbdb-bli/{new_id}/draft/files", headers=h)
    assert res.status_code == 200
    assert len(res.json["entries"]) == 0

    # Import files from previous version
    res = client.post(f"/mbdb-bli/{new_id}/draft/actions/files-import", headers=h)
    assert res.status_code == 201
...
def test_import_files_no_version(client, search_clear, location, headers):
    ...
    res = client.post(f"/mbdb-bli/{id_}/draft/actions/files-import", headers=h)
