...
from mbdb_bli.proxies import current_service
from mbdb_bli.records.api import MbdbBliDraft, MbdbBliRecord
from mbdb_bli.resources.records.config import MbdbBliResourceConfig
...
def _create_and_publish(client, input_data):
    ...
    # Create the draft
    response = client.post(MbdbBliResourceConfig.url_prefix, json=input_data)
    ...
    response = client.post(
        f"{ MbdbBliResourceConfig.url_prefix}{recid}/draft/actions/publish"
    )
...
def test_publish_draft(client, input_data, search_clear):
    ...
    response = client.get(f"{ MbdbBliResourceConfig.url_prefix}{recid}/draft")

    assert response.status_code == 404

    # Check record exists
    response = client.get(f"{ MbdbBliResourceConfig.url_prefix}{recid}")
...
def test_search_versions(client, input_data, search_clear):
    ...
    recid = _create_and_publish(client, input_data)
    MbdbBliDraft.index.refresh()

    # Check draft does not exists anymore
    res = client.get(f"{ MbdbBliResourceConfig.url_prefix}{recid}/versions")
...
def test_create_publish_new_revision(
    ...
    response = client.put(
        f"{ MbdbBliResourceConfig.url_prefix}{recid}/draft", json=edited_input_data
    )
...
def test_redirect_to_latest_version(client, input_data, search_clear):
    ...
    response = client.post(f"{ MbdbBliResourceConfig.url_prefix}{recid}/versions")
    ...
    client.put(f"{ MbdbBliResourceConfig.url_prefix}{recid_2}/draft", json=input_data)

    # Publish it to check the increment in version
    response = client.post(
        f"{ MbdbBliResourceConfig.url_prefix}{recid_2}/draft/actions/publish"
    )
    latest_version_self_link = response.json["links"]["self"]

    # Read a previous versions latest
    response = client.get(f"{ MbdbBliResourceConfig.url_prefix}{recid}/versions/latest")
...
def test_list_drafts(client, input_data, vocab_cf, search_clear):
    assert len(client.get(MbdbBliResourceConfig.url_prefix).json["hits"]["hits"]) == 0
    assert (
        len(client.get(f"user{ MbdbBliResourceConfig.url_prefix}").json["hits"]["hits"])
        == 0
    )
    ...
    MbdbBliDraft.index.refresh()
    MbdbBliRecord.index.refresh()
    assert len(client.get(MbdbBliResourceConfig.url_prefix).json["hits"]["hits"]) == 1
    assert (
        len(client.get(f"user{ MbdbBliResourceConfig.url_prefix}").json["hits"]["hits"])
        == 0
    )
