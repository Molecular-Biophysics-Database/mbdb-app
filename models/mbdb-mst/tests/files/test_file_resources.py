...
def published_id(client, location, headers):
    ...
    # Initialize files upload
    res = client.post(
        f"/mbdb-mst/{id_}/draft/files", headers=h, json=[{"key": "test.pdf"}]
    )
    assert res.status_code == 201
    assert res.json["entries"][0]["key"] == "test.pdf"
    ...
    # Commit the file
    res = client.post(f"/mbdb-mst/{id_}/draft/files/test.pdf/commit", headers=h)
    assert res.status_code == 200
    assert res.json["key"] == "test.pdf"
    assert res.json["status"] == "completed"

    # Publish the record
    res = client.post(f"/mbdb-mst/{id_}/draft/actions/publish", headers=h)
...
def test_files_publish_flow(client, search_clear, location, headers):
    ...
    h = headers
    # Create a draft
    res = client.post("/mbdb-mst/", headers=h, json={"metadata": {"title": "Test"}})
    assert res.status_code == 201
    id_ = res.json["id"]
    ...
    res = client.get(f"/mbdb-mst/{id_}/files", headers=h)
    assert res.status_code == 200
    assert res.json["entries"][0]["key"] == "test.pdf"
    assert res.json["entries"][0]["status"] == "completed"

    # Edit the record
    res = client.post(f"/mbdb-mst/{id_}/draft", headers=h)
    assert res.status_code == 201
...
def test_import_files(client, search_clear, location, headers, published_id):
    ...
    # New version
    res = client.post(f"/mbdb-mst/{id_}/versions", headers=h)
    assert res.status_code == 201
    new_id = res.json["id"]

    # Check new version files
    res = client.get(f"/mbdb-mst/{new_id}/draft/files", headers=h)
    assert res.status_code == 200
    assert len(res.json["entries"]) == 0

    # Import files from previous version
    res = client.post(f"/mbdb-mst/{new_id}/draft/actions/files-import", headers=h)
    assert res.status_code == 201
...
def test_import_files_no_version(client, search_clear, location, headers):
    ...
    res = client.post(f"/mbdb-mst/{id_}/draft/actions/files-import", headers=h)
