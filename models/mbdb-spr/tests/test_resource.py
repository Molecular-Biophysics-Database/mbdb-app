...
def _create_and_publish(client, input_data):
    ...
    # Create the draft
    response = client.post(MbdbSprResourceConfig.url_prefix, json=input_data)
...
def test_publish_draft(client, input_data, search_clear):
    ...
    response = client.get(f"{ MbdbSprResourceConfig.url_prefix}{recid}/draft")

    assert response.status_code == 404

    # Check record exists
    response = client.get(f"{ MbdbSprResourceConfig.url_prefix}{recid}")
...
def test_search_versions(client, input_data, search_clear):
    ...
    # Check draft does not exists anymore
    res = client.get(f"{ MbdbSprResourceConfig.url_prefix}{recid}/versions")
...
def test_create_publish_new_revision(
    ...
    response = client.post(f"{ MbdbSprResourceConfig.url_prefix}{recid}/draft")
...
def test_redirect_to_latest_version(client, input_data, search_clear):
    ...
    response = client.post(f"{ MbdbSprResourceConfig.url_prefix}{recid}/versions")
    ...
    client.put(f"{ MbdbSprResourceConfig.url_prefix}{recid_2}/draft", json=input_data)
    ...
    response = client.get(f"{ MbdbSprResourceConfig.url_prefix}{recid}/versions/latest")
...
def test_list_drafts(client, input_data, vocab_cf, search_clear):
    assert len(client.get(MbdbSprResourceConfig.url_prefix).json["hits"]["hits"]) == 0
    assert (
        len(client.get(f"user{ MbdbSprResourceConfig.url_prefix}").json["hits"]["hits"])
        == 0
    )
    ...
    MbdbSprDraft.index.refresh()
    MbdbSprRecord.index.refresh()
    assert len(client.get(MbdbSprResourceConfig.url_prefix).json["hits"]["hits"]) == 1
    assert (
        len(client.get(f"user{ MbdbSprResourceConfig.url_prefix}").json["hits"]["hits"])
        == 0
    )
