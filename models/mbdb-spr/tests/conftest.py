...
from mbdb_spr.resources.files.config import (
    MbdbSprFileDraftResourceConfig,
    MbdbSprFileResourceConfig,
)
...
BASE_URLS = {
    ...
    "base_html_url": "/spr/",
    "base_files_url": MbdbSprFileResourceConfig.url_prefix.replace(
        "<pid_value>", "{id}"
    ),
    "base_draft_files_url": MbdbSprFileDraftResourceConfig.url_prefix.replace(
        "<pid_value>", "{id}"
    ),
}
...
def sample_record(app, db, input_data):
    ...
    with UnitOfWork(db.session) as uow:
        record = MbdbSprRecord.create(input_data)
...
def sample_records(app, db, sample_metadata_list):
    ...
    with UnitOfWork(db.session) as uow:
        ...
        for sample_metadata in sample_metadata_list:
            record = MbdbSprRecord.create(sample_metadata)
            uow.register(RecordCommitOp(record, current_service.indexer, True))
...
def sample_draft(app, db, input_data):
    with UnitOfWork(db.session) as uow:
        record = MbdbSprDraft.create(input_data)
        uow.register(RecordCommitOp(record, current_service.indexer, True))
        uow.commit()
        return record
...
def published_record_factory(record_service):
    ...
    def create_record(input_data):
        ...
        record = MbdbSprRecord.pid.resolve(published_record["id"])
        return record
